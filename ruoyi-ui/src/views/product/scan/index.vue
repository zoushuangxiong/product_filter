      ...
      <el-form label-position="top" @submit.prevent="start">
        ...
        <el-form-item label="商品平台（必选）">
          <el-select v-model="form.platform" placeholder="请选择淘宝/天猫或1688" :disabled="active || busy || libraryApplying" style="width:100%" @change="changePlatform">
            ...
            <el-form-item label="商品 CSV 文件">
              <div class="csv-import">
                ...
                <p class="upload-limit">CSV 格式 · 最多 10,000 条 · 不超过 20 MB</p>
              </div>
            </el-form-item>

            <p class="muted import-note">每个任务只检测一个平台。模板中“商品链接”必填，支持完整链接或数字 ID，其他列选填。</p>
          </section>
          <section class="rules-section">
            ...
            <div class="filter-word-hint"><strong>填写规则</strong><span>每行一个词，按回车换行；逗号、顿号、空格不会分隔词语。每份最多 500 个词，每词最多 100 字符，总计最多 30,000 字符。</span></div>
          </section>
        </div>
        <div class="scan-actions">
          <div class="action-status"><el-icon><Document /></el-icon><span v-if="importedName">已导入 <strong>{{ importedCount }}</strong> 条商品记录</span><span v-else>选择平台并导入 CSV 后，即可开始检测</span></div>
    ...
    <el-card shadow="never" class="results" v-loading="resultsLoading || historyLoading" element-loading-text="正在加载检测结果…">
      ...
      <template v-if="job">
        <div class="task-progress" aria-live="polite">
          <div class="progress-heading"><strong>{{ progress.title }}</strong><span>已处理 {{ progress.processed }} / {{ job.products.length }} 个商品</span></div>
          <el-progress :percentage="progress.percent" :stroke-width="10" :color="active ? '#409eff' : '#909399'"/>
          <div class="progress-details">
            <span v-if="progress.current">商品 ID：{{ progress.current.itemId }} · {{ label(progress.current.state) }}<template v-if="progress.current.state === 'SCANNING'"> · 图片已处理 {{ progress.current.pictures.filter(p => ['DONE','FAILED'].includes(p.state)).length }} / {{ progress.current.pictures.length }} 张</template></span>
            <span v-else>{{ label(job.state) }} · 图片已处理 {{ processedImages }} / {{ totalImages }} 张</span>
        ...
        <div class="summary"><div>商品总数<strong>{{ job.products.length }}</strong></div><div>命中词库<strong>{{ job.products.filter(p => p.verdict === 'MATCHED').length }}</strong></div><div>待复核 / 未完成<strong>{{ job.products.filter(p => p.incomplete || p.state !== 'DONE').length }}</strong></div><div>已通过<strong>{{ job.products.filter(eligible).length }}</strong></div></div>
        <div class="toolbar"><el-select v-model="filter" style="width:180px"><el-option label="全部结果" value="ALL"/><el-option label="命中词库" value="MATCHED"/><el-option label="待复核 / 未完成" value="REVIEW"/><el-option label="通过（可导出）" value="ELIGIBLE"/></el-select><el-button @click="download(false)">导出全部 CSV</el-button><el-button type="primary" plain @click="download(true)">导出通过项 CSV</el-button></div>
        ...
        <el-table :data="pagedProducts" row-key="itemId" class="result-table">
          ...
          <el-table-column label="结果与证据" min-width="300"><template #default="{row}"><el-tag :type="resultStatus(row).type">{{ resultStatus(row).text }}</el-tag> <el-tag v-if="row.incomplete" type="warning">内容未完整检测</el-tag><p v-if="row.titleHits.length">标题命中：{{ row.titleHits.join('、') }}</p><p v-if="row.error" class="error">{{ errorText(row.error) }}</p><div class="evidence"><template v-for="(pic,index) in row.pictures" :key="index"><el-button v-if="pic.hits.length || hasLowConfidence(pic) || pic.qrCodes" :type="pic.hits.length || pic.qrCodes ? 'danger' : 'warning'" plain size="small" :disabled="!pic.ocr" @click="openPicture(row,index)">{{ pic.kind === 'MAIN' ? '主图' : '详情图' }} {{ pic.index }} · {{ pic.qrCodes ? `二维码 ${pic.qrCodes} 个` : '' }}{{ pic.qrCodes && (pic.hits.length || hasLowConfidence(pic)) ? ' / ' : '' }}{{ pic.hits.length ? '命中' : '' }}{{ pic.hits.length && hasLowConfidence(pic) ? ' / ' : '' }}{{ hasLowConfidence(pic) ? '低置信度待核查' : '' }}</el-button></template></div><div class="scan-warnings"><p v-for="warning in visibleWarnings(row)" :key="warning">{{ warning }}</p><template v-for="(pic,index) in row.pictures" :key="index"><p v-if="pic.error">{{ pic.kind === 'MAIN' ? '主图' : '详情图' }} {{ pic.index }}：{{ errorText(pic.error) }}</p></template></div></template></el-table-column>
        ...
        <pagination v-show="products.length > 0" class="result-pagination" :total="products.length" v-model:page="resultPage" v-model:limit="resultPageSize" :page-sizes="[10, 20, 50, 100]" :auto-scroll="false"/>
...
function resultStatus(p){
  ...
  if(p.incomplete || p.state!=='DONE')return {text:'待复核',type:'warning'}
...
// 按整个任务计算进度，不受分页或结果筛选影响；取消及中断不算已处理。
const progress = computed(() => {
  const items = job.value?.products || []
  const processed = items.filter(p => ['DONE', 'FAILED'].includes(p.state)).length
  const failed = items.filter(p => p.state === 'FAILED').length
  const index = items.findIndex(p => ['FETCHING', 'SCANNING'].includes(p.state))
  const current = active.value && index >= 0 ? items[index] : null
  let title = label(job.value?.state) || '检测进度'
  if (job.value?.state === 'QUEUED') title = '等待开始检测'
  else if (current) title = `正在检测第 ${index + 1} 个商品，共 ${items.length} 个`
  else if (job.value?.state === 'RUNNING') title = processed === items.length ? '正在汇总检测结果' : '正在准备检测下一个商品'
  else if (job.value?.state === 'COMPLETED') title = '检测已结束'
  return { title, current, processed, failed, percent: items.length ? Math.floor(processed * 100 / items.length) : 0 }
})
...
// 先筛选再分页；轮询同一任务时保留页码，统计和导出仍使用整个任务。
const pagedProducts = computed(() => products.value.slice((resultPage.value - 1) * resultPageSize.value, resultPage.value * resultPageSize.value))
watch([filter, resultPageSize, () => job.value?.id], () => { resultPage.value = 1 })
// 复核后商品可能移出当前筛选结果，避免停留在已不存在的末页。
watch(() => products.value.length, total => {
  resultPage.value = Math.min(resultPage.value, Math.max(1, Math.ceil(total / resultPageSize.value)))
})
...
async function load(id,showLoading=true){
  ...
  try{
    ...
    job.value=data;confidenceThreshold.value=data.rules?.confidenceThreshold ?? 0.6;poll()
  }catch(e){
...
async function start(){
  ...
  try{const {data}=await api.createTask({...form,confidenceThreshold:confidenceThreshold.value,items:form.items.split(/\r?\n/).map(s=>s.trim()).filter(Boolean),sourceCsv});job.value=data;localStorage.setItem('product-scan-rules',JSON.stringify({titleWords:form.titleWords,imageWords:form.imageWords,detectPhones:form.detectPhones,detectQrCodes:form.detectQrCodes}));await refresh();poll()}catch(e){error(e)}finally{busy.value=false}
}
...
async function importFile(event){
  ...
  try{
    ...
    const original=decodeCsv(await file.arrayBuffer()).replace(/^\uFEFF/,'')
...
async function changeConfidenceThreshold(value) {
  ...
  try {
    const { data } = await api.updateConfidenceThreshold(id, value)
    if (!disposed && job.value?.id === id && requestSequence === seq) {
      job.value = data
      confidenceThreshold.value = data.rules.confidenceThreshold
    }
  } catch (e) {
    error(e)
...
.summary>div:nth-child(2) strong{color:var(--el-color-danger)}
.summary>div:nth-child(3) strong{color:var(--el-color-warning-dark-2)}
.summary>div:nth-child(4) strong{color:var(--el-color-success)}
