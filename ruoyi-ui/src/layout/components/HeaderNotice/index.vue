...
import { isProjectNotice } from '@/utils/projectContent'
...
function loadNoticeTop() {
  ...
  listNoticeTop().then(res => {
    noticeList.value = (res.data || []).filter(isProjectNotice)
    const hiddenUnreadCount = (res.data || []).filter(n => !isProjectNotice(n) && !n.isRead).length
    unreadCount.value = res.unreadCount !== undefined
      ? Math.max(0, res.unreadCount - hiddenUnreadCount)
